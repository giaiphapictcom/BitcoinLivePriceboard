...
public class MarketsAsyncTask extends GetAsyncTask {
    ...
    public MarketsAsyncTask(Activity context) {
        super(context, constant.BASE_URL + "/market/bitcoin/?convert="+constant.CURRENCY_CONVERT );
    }

    @Override
    protected String doInBackground(String... params){
        String result = api.getContent(constant.BASE_URL + "/market/bitcoin/?convert="+constant.CURRENCY_CONVERT);
        if(result!=null) {
            try {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    item.put("number", Integer.toString(i+1)  );
    ...
    protected void onPostExecute(String result) {
        ...
        if(markets.size() > 0) {
            ListView MarkeListView = (ListView) this.context.findViewById(R.id.Markets);
